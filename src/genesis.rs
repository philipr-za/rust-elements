...
use crate::{
    confidential, script, AssetId, Block, BlockExtData, BlockHash, BlockHeader, PackedLockTime,
...
fn commit_to_network_parameters(network: Network) -> Vec<u8> {
    ...
    sha256::Hash::from_engine(eng).to_vec()
}
        ...
        network => {
            ...
            let merkle_root: sha256d::Hash =
                if let Some(asset_tx) = liquid_genesis_asset_tx(network) {
                    ...
                    let tx_hashes = vec![tx.txid().as_hash(), asset_tx.txid().as_hash()];
                    bitcoin::util::hash::bitcoin_merkle_root(tx_hashes.into_iter())
                        .expect("merkle root")
                } else {
                    tx.txid().as_hash().into()
                };
    ...
    fn genesis_block_hash() {
        ...
        let genesis_block =
            crate::genesis::genesis_block(NetworkParams::new(Network::Liquidtestnet).unwrap())
                .unwrap();
        println!("{}", genesis_block.block_hash());
        println!("{}", genesis_block.header.merkle_root);
        assert_eq!(
            genesis_block.block_hash().as_ref(),
