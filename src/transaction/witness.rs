...
impl Witness {
    ...
    pub fn from_hex<I, T>(iter: I) -> Result<Self, hex::DecodeVariableLengthBytesError>
    where
...
impl<T: core::borrow::Borrow<[u8]>> PartialEq<rc::Rc<[T]>> for Witness {
    fn eq(&self, rhs: &rc::Rc<[T]>) -> bool { *self == **rhs }
}

impl<T: core::borrow::Borrow<[u8]>> PartialEq<Witness> for rc::Rc<[T]> {
    fn eq(&self, rhs: &Witness) -> bool { *rhs == *self }
}

#[cfg(target_has_atomic = "ptr")]
impl<T: core::borrow::Borrow<[u8]>> PartialEq<sync::Arc<[T]>> for Witness {
    fn eq(&self, rhs: &sync::Arc<[T]>) -> bool { *self == **rhs }
}

#[cfg(target_has_atomic = "ptr")]
impl<T: core::borrow::Borrow<[u8]>> PartialEq<Witness> for sync::Arc<[T]> {
    fn eq(&self, rhs: &Witness) -> bool { *rhs == *self }
}
...
pub mod error {
    ...
    use crate::encoding::CompactSizeDecoderError;
    ...
    impl fmt::Display for WitnessDecoderError {
        fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
            ...
            match self.0 {
                E::LengthPrefixDecode(..) => f.write_str("vec decoder error"),
                E::UnexpectedEof(..) => f.write_str("decoder error"),
            }
        }
    }
...
mod test {
    ...
    fn partial_eq() {
        ...
        // Rc<[T]>
        let container: rc::Rc<[&[u8]]> = vec![DATA_1, DATA_2].into();
        let different: rc::Rc<[&[u8]]> = vec![DATA_2, DATA_1].into();
        ck!(Witness::from(&*container), container, different);

        // Arc<[T]>
        let container: sync::Arc<[&[u8]]> = vec![DATA_1, DATA_2].into();
        let different: sync::Arc<[&[u8]]> = vec![DATA_2, DATA_1].into();
        ck!(Witness::from(&*container), container, different);
    }
